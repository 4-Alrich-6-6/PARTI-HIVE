(function () {
    ...
    const loadProfile = async () => {
        ...
        if (!user) return { displayName: "", program: "", avatar: "", role };

        const { data, error } = await hiveSupabase
            .from('USER')
            .select('userDisplayName, avatarPath, progId, deptId')
            .eq('userId', user.id)
            .maybeSingle();

        if (error || !data) return { displayName: "", program: "", avatar: "", role };

        return {
            displayName: data.userDisplayName || "",
            program: data.progId || data.deptId || "",
    ...
    const init = () => {
        ...
        let currentProfile = { displayName: "", program: "", avatar: "", role };
        loadProfile().then(profile => {
            currentProfile = profile;
            applyToSidebar(profile);
        });
        ...
        avatarInput.addEventListener("change", async () => {
            ...
            const { error: uploadErr } = await hiveSupabase.storage
                .from('profilePicture')
            ...
            const { data: urlData } = hiveSupabase.storage
                .from('profilePicture')
                .getPublicUrl(filePath);

            const publicUrl = urlData?.publicUrl || '';

            const { error: dbErr } = await hiveSupabase
                .from('USER')
                .update({ avatarPath: publicUrl })
                .eq('userId', user.id);

            if (dbErr) {
                console.error('Failed to save avatar path:', dbErr.message);
            }
        });
